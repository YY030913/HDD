<?php
session_start();
#后台管理文件，使用前请先配置下面用户名及密码


#将这里的admin用户名改成你喜欢的
$adminuser = 'itjesse';

#这是后台密码，默认为admin，你可以改成任意你喜欢的
$adminpassword = 'zyb940708';
...
if($_POST['adminuser'] && ($_POST['adminpassword'] == $admin_data['password']) && ($_POST['adminuser'] == $admin_data['username']) ) {
	$_SESSION['_wcc_adminuser'] = $_POST['adminuser'];
	echo '<script>window.location.href = window.location.href;</script>';
}


if($_SESSION['_wcc_adminuser']==$admin_data['username']) {
?>
<style>
.chuncaidiv {
...
<div class="wrap">
<?php
$optionpath = substr(__FILE__, 0, -14);
require $optionpath.'config.inc.php';
...
if($_GET['del']){
	...
	if(preg_match($preg, $id) ) {
		...
		exit('要删自带的萌妹子？移植版我懒得做这个删除了，谁要谁来做。<a href="./sm-options.php">点击返回</a>');
		...
		unset($wcc['ccs'][$id]);
		update_option('sm-weichuncai', $wcc);
		echo '<script>window.location.href="sm-options.php";</script>';
	}
}
if($_POST['additional']){
...
<p><a href="sm-options.php"><?php _e('通用设置', 'weichuncai'); ?></a> | <a href="sm-options.php?cp=1"><?php _e('创建伪春菜', 'weichuncai'); ?></a></p>
<hr>
<?php 
if($_GET['cp'] != '1') {
...
<?php
	foreach($wcc['ccs'] as $k=>$v){
		if($v == $wcc['defaultccs']){
			echo '<div style="" class="chuncaidiv"><img src="'.$weichuncai_base_data['weichuncai_path'].'skin/'.$v.'/face1.gif"><p><input type="radio" name="defaultccs" value="'.$v.'" checked> '.$v.'</p></div>';
		}else{
			$isdelcc = __("删除？", "weichuncai");
			echo '<div class="chuncaidiv" style=""><img src="'.$weichuncai_base_data['weichuncai_path'].'skin/'.$v.'/face1.gif"><p><input type="radio" name="defaultccs" value="'.$v.'"> '.$v.'(<a href="sm-options.php?del=del&ccsid='.$k.'">'.$isdelcc.'</a>)</p></div>';
		}
	}
	...
	if( !empty($wcc['userdefccs']) ) {
		foreach( $wcc['userdefccs'] as $k=>$v ) {
			if( 'userdefccs_'.$k == $wcc['defaultccs'] ) {
				...
				$isdelcc = __("删除？", "weichuncai");
				echo '<div class="chuncaidiv" style=""><img src="'.$v['face'][0].'"><p><input type="radio" name="defaultccs" value="userdefccs_'.$k.'"> '.$k.'(<a href="./sm-options.php?del=del&ccsid=userdefccs_'.$k.'">'.$isdelcc.'</a>)</p></div>';
			}
		}
	}
?>
...
<p><?php _e('提示：复制图片地址到下面文本框。或者你可以到', 'weichuncai'); ?><a href="javascript:;" onclick="alert('移植版不做上传操作。');"><?php _e('上传图片', 'weichuncai'); ?></a><?php _e('先进行上传。图片应小于160像素。', 'weichuncai'); ?></p>
...
<form action="" method="post">
<ul>
用户名：<input type="text" name="adminuser" value="">
</ul>
<ul>
密&nbsp;&nbsp;码：<input type="password" name="adminpassword" value="">
</ul>

<ul>
提&nbsp;&nbsp;交：<input type="submit" name="sub" value="登陆">
</ul>
</form>
